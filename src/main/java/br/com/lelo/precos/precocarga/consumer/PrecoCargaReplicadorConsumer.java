...
@Component
public class PrecoCargaReplicadorConsumer {
	...
	public void go() {
		...
		String topicName = TopicEnum.TOPIC_REPLICADOR.getTopicName();
			...
			public void run() {
				try (KafkaConsumer<String, PrecoCarga> consumer = properties.createConsumer(topicName)) {
					...
					while (true) {
						...
						log.info(PrecoCargaReplicadorConsumer.class.getSimpleName() + " Looking for message ...");

						for (ConsumerRecord<String, PrecoCarga> record : consumer.poll(10)) {
							log.info("[replicador] item: " + record.value());
						}
